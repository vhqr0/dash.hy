...
(defn lodash-name-add-prefix [lodash-name prefix]
  (cond (s.starts-with? lodash-name "__") (s.concat-str "__" prefix (cut lodash-name 2 None))
        (s.starts-with? lodash-name "_")  (s.concat-str "_" prefix (cut lodash-name 1 None))
        True (s.concat-str prefix lodash-name)))
...
(defn load-name [prefix module name]
  (let [lodash-name (s.concat-str prefix (lodash-name-from-name name))]
